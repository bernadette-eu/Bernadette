//=======================================================================
//
// Copyright (C) 2022, Lampros Bouranis, Marie Curie Fellow <bouranis@aueb.gr>
...
functions {
	...
	matrix to_triangular(vector x, int K) {
		...
		matrix[K, K] y = rep_matrix(0.0, K, K);
		int pos = 1;
									  ...
									  )
    {
      ...
      int A       = x_i[1];       // Number of age groups
	  int n_obs   = x_i[2];       // Length of time horizon
...
data {							  
  ...
  real p_phi_sd;                    // Half-Cauchy[scale] on phi, in the overdispersed count model
...
parameters {
  ...
  real<lower = 0, upper = 1> sigmaBM_u[A];   // Standard deviation of GBM
  real<lower = 0> disp;                      // disp = 1/sqrt(phi); Likelihood variance parameter
...
transformed parameters{
  ...
  for(col in 1:A) {
	  ...
	  sigmaBM[col] = p_sigmaBM[col] * tan( pi() * 0.5 * sigmaBM_u[col] ); // Equivalent to cauchy(0, p_sigmaBM[col])
