...
transformed parameters{
  ...
  matrix<lower = 0>[n_weeks, A] beta_weekly; // Weekly Effective contact rate, beta_weekly = exp(eta_weekl																				
  real<lower = 0> beta_N[n_weeks*A];         // Daily Effective contact rate
  ...
  eta_weekly[1,] = to_row_vector(eta_init);  
...
model {
  ...
  sigmaBM   ~ normal(0, p_sigmaBM[1]);
  phiD      ~ exponential(p_phi);
  L_raw     ~ std_normal();
