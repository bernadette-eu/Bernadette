...
`Bernadette` package [@bernadette] for the
...
`EpiEstim` [@Cori2013; @epiestim] offers a data-driven Bayesian framework for the
...
by a renewal equation. The `epidemia` package [@epidemia] links observed areal data to latent infections,
...
offered by the probabilistic programming language `Stan` [@carpenter2017stan] to specify a model and draw from the
...
program and then it is fit using sampling methods from `rstan` [@rstan].
