...
The model described below can be computationally expensive to implement and so we restrict the analysis to a shorter time horizon. We analyse the subset from 2020-10-02 to 2021-01-07 (98 days):
...
```{r mortality_time_series_subset, echo = FALSE, fig.height=5, fig.width=7}
sub <- age_specific_mortality_counts[c(3, 6:ncol(age_specific_mortality_counts))]
mortality_df_long <- stats::reshape(sub,
                                    direction = "long",
                                    varying   = list(names(sub)[2:ncol(sub)]),
...
The transmission of COVID-19 is modeled through an age-stratified deterministic Susceptible-Exposed-Infectious-Removed (SEIR) compartmental model \citep{anderson} with Erlang-distributed latent and infectious periods. In particular, we introduce Erlang-distributed stage durations in the Exposed and Infected compartments and relax the mathematically convenient but unrealistic assumption of exponential stage durations by assuming that each of the Exposed and Infected compartments are defined by two stages, with the same rate of loss of latency ($\tau$) and infectiousness ($\gamma$) in both stages.
...
The number of individuals in each compartment is scaled by the total population $\mathbb{N} = \sum_{\alpha = 1}^{A}N_{\alpha}$, so that the sum of all compartments equals to one \citep{riou2}.
...
breaking down the transmission rate matrix into its biological and social components \citep{baguelin, knock}: the social component is represented by the average number of contacts between individuals of age group $\alpha$ and age group $\alpha'$ viThe number of individuals in each compartment is scaled by the total populationa the contact matrix element $C_{\alpha,\alpha'}$; 
...
The unknown quantities that need to be inferred are $\theta= (x^{\alpha\alpha}_{0}, x^{\alpha\alpha}_{1:T}, C_{\alpha,\alpha'}, \sigma_{x,\alpha}, \rho, \phi)$ for $\alpha \in \{1,\ldots,A\}$ for $(\alpha,\alpha') \in \{1,\ldots,A\}^2$
...
\toprule
\textbf{Symbol} & \textbf{Description} & \textbf{Prior \& source} \\
\hline	
\multicolumn{3}{l}{\textbf{Transmission model parameters}} \\						
$\rho$ & Initial proportion of exposed (at time $t_0$) & Beta with $\mathbb{E}[\rho] = 0.1$, $\mathbb{V}[\rho] = 0.05\cdot\mathbb{E}[\rho]$.\\

$\tilde{L}_{i,j}$ & Lower-triangular random matrix & $\tilde{L}_{i,j} \sim \operatorname{Normal}(0,1),~(i,j) \in \{1, \ldots, A\}^2, i \leq j$.
\\
$L_{i,j}$ & Entries of the Cholesky factor $L$ & $L_{i,j} = L^{synth}_{i,j} + (0.05\cdot L^{synth}_{i,j})\cdot \tilde{L}_{i,j},~(i,j) \in \{1, \ldots, A\}^2, i \leq j$.\\
 \\
$x^{\alpha\alpha}_{0}$ & MBM model Brownian motion, at time $t_0$ & $\operatorname{Normal}(0,5^2)$ \\
$x^{\alpha\alpha}_{1}$ & MBM model Brownian motion, at time $t_1$ & $\operatorname{Normal}(0,5^2)$ \\
$x^{\alpha\alpha}_{t}$ & MBM model Brownian motion at time $t$&  $x^{\alpha\alpha}_{t}\mid x^{\alpha\alpha}_{t - 1}, \sigma^2_{x,\alpha} \sim \operatorname{Normal}(x^{\alpha\alpha}_{t - 1}, \sigma^2_{x,\alpha}),~t = 2,\ldots,T.$ \\
$\sigma_{x}$ & Volatility of the Brownian motion in the SBM model & $\operatorname{Half-Normal}(0,4)$.\\
$\sigma_{x,\alpha}$ & Volatility of the Brownian motion in the MBM model & $\operatorname{Half-Normal}(0,4)$\\
\hline	
\multicolumn{3}{l}{\textbf{Observation model parameters}} \\	
$\phi$ & Negative Binomial over-dispersion & $\operatorname{Half-Normal}(0,5)$ for the MBM model, Austria. Otherwise, Exp(0.2). \\	
\hline
\multicolumn{3}{l}{\textbf{Parameters assumed known}} \\	
$\widehat{\text{IFR}}_{\alpha}$ & Infection-fatality rate & \\
$d_E$ & Mean Incubation period & 3 days.\\
$d_I$ & Mean Infection period  & 4 days.\\
\bottomrule
\end{tabular}
\end{table}
...
Sample from the posterior distribution of the parameters using the Hamiltonian Monte Carlo algorithm implemented in Stan - Note that a longer time horizon and/or a greater number of age groups in ```y_data``` will lead to increased CPU time:
...
Visualise the posterior distribution (age-specific or aggregated daily estimates) of the infection counts:
...
Visualise the posterior distribution (age-specific or aggregated daily estimates) of the mortality counts:
